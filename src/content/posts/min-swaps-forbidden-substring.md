...
2. **$T$ 相邻字符互不相同 $\Rightarrow$ 哪里出现两个相邻相同字符，那一段就绝不可能是 $T$。** 后面"放心破坏目标、不产生新的 $T$"全靠它。
...
- `#0` 的**最后一格** $\to$ 字符是 $T_{m-1}$（这里 $=\texttt a$）；
- `#3` 的**第一格** $\to$ 字符是 $T_0$（这里 $=\texttt a$）。
...
- **路径 1（全柔性）**：$0\to3\to4\to5\to6\to7$，共 5 块、全为柔性、0 刚性。
- **路径 2（含刚性边）**：$0\to4\to5\to6\to7$，共 4 块、其中 1 刚性。
...
一次交换最多改 2 个下标 $\Rightarrow$ $k$ 次交换最多改 $2k$ 个下标。每个块都得至少被改一个下标才能摧毁，且不同块各用各的下标。所以
...
> **要补的几个细节（选读）。**
>
> **(1) 安全格不止一个。** 上面是在"倒数第二格 $b\to a$"和"最后一格 $a\to b$"制造相同相邻字符。一个柔性块的两头附近一共有 **4 个**这样的安全候选格（记 $x=T_{m-2},y=T_{m-1},u=T_0,v=T_1$；在例子 `abababa` 里 $x=b,\,y=a,\,u=a,\,v=b$）：
>
> | 候选 | 在哪一格改 | 怎么改 | 例子里就是 |
> |---|---|---|---|
> | A | 块首出现的倒数第二格 | $x\to y$ | $b\to a$ |
> | B | 块首出现的最后一格 | $y\to x$ | $a\to b$ |
> | C | 块尾出现的第二格 | $v\to u$ | $b\to a$ |
> | D | 块尾出现的第一格 | $u\to v$ | $a\to b$ |
>
> 两块合用一次交换，就挑一对"改法正好互补"（一个 $x\to y$、另一个 $y\to x$，交换两端即可对调）的。刚才用的就是 A+B。
>
> **(2) 多次交换别互相干扰。** 若两次交换的安全格贴在一起，一处的改动可能毁掉另一处刚弄出的相同相邻字符。办法：把所有柔性块**按位置排序，左半统一用一种改法、右半统一用互补改法，再跨左右配对**——这样全串安全格的排布是"一种…一种…另一种…另一种"，不会在交界处冲突。
>
> **(3) 刚性块更简单。** 刚性块只能动它那唯一的 $c$ 格，把 $c$ 改掉即可；它和一个柔性块也能凑成一次交换。单独一个刚性块、单独一个柔性块，也都各用一次交换就能解决。

下图是例子路径 2 $u=3,f=1$ 的配法（红=刚性，蓝=柔性，黄=一次交换）：

![路径 2 的配对：刚性块 #0-#3 配柔性块 #4 用一次交换、#5 配 #6 一次，共 2 次 = max(⌈4/2⌉, 1)](./images/fig-pairing.png)

### 5.4 例子的两条路径

| 路径 | 划分出的块 | $u$ | $f$ | $C=\max(\lceil\frac{u+f}{2}\rceil,f)$ |
|---|---|---|---|---|
| 1 全柔性 | 柔`{#0,#1,#2}` 柔`{#3}` 柔`{#4}` 柔`{#5}` 柔`{#6}` | 5 | 0 | $\max(3,0)=3$ |
| 2 用刚性 | **刚**`{#0..#3}` 柔`{#4}` 柔`{#5}` 柔`{#6}` | 3 | 1 | $\max(2,1)=\mathbf2$ |

**最优 = 2 次交换**，走路径 2。这就是为什么要引入刚性边：它把"柔性块`{#0,#1,#2}` + 孤立的`{#3}`"两块**合并成一块**，块数从 5 降到 4，把配对下界从 3 压到 2，代价只是多 1 个刚性块（而 $f=1$ 还没成为瓶颈）。
