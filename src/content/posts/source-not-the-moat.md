...
但能自动运行的循环不一定健康。前面用来判断个人是否仍有掌控的三个条件，也适用于整个工作流：它是否**可理解、可干预、可验证**。我把它们缩写为 CCV（Comprehensible、Controllable、Verifiable）。如果决策藏在黑箱里，边界不能随新证据调整，完成声明又没有独立证据，那么循环跑得越快，人失去掌控的速度也越快。
