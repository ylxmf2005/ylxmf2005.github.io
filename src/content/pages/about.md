...
你好，我是李易峰（Ethan Li）。我目前是香港理工大学计算机科学本科生，曾在 PolyU NLP Group 担任本科研究助理，目前在腾讯 PCG 机器学习平台实习。
...
- **智能体评测**：超越 Benchmark 分数，研究 Agent 在真实任务中的判断、轨迹与失败模式。
- **Agent Harness 与基础设施**：构建上下文、记忆、Skills、工具、Sandbox 与运行时，让 Agent 更可靠、更有效。
- **Coding Agent**：设计能够规划、执行、验证并持续演化的软件 Agent 回路，同时保留 Human/Expert in the Loop。

## 研究与工程

我的兴趣位于研究与工程的交界处。我喜欢把研究想法实现为能够运行、观察和评估的系统，而不只是停留在独立的原型上。

在腾讯 PCG，我主要参与 Agent 系统与模型基础设施的建设，关注系统可靠性、评测、性能优化与生产落地。
